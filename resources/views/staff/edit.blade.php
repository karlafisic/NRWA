...
<div class="container mt-4">
    ...
    <form action="{{ route('staff.update', $staff->staff_id) }}" method="POST">
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="text" id="first_name" name="first_name" class="form-control" 
                    value="{{ $staff->first_name }}" required maxlength="45">
            </div>

            <div class="col-md-6 mb-3">
                <label for="last_name" class="form-label">Last Name:</label>
                <input type="text" id="last_name" name="last_name" class="form-control" 
                    value="{{ $staff->last_name }}" required maxlength="45">
            </div>
        </div>
            ...
            <select id="address_id" name="address_id" class="form-select" required>
                @foreach($addresses as $address)
                    <option value="{{ $address->address_id }}"
                        @if($staff->address_id == $address->address_id) selected @endif>
        ...
        <div class="mb-3">
            ...
            <textarea id="picture" name="picture" class="form-control" rows="3">{{ $staff->picture }}</textarea>
            ...
            <select id="store_id" name="store_id" class="form-select" required>
                @foreach($stores as $store)
                    <option value="{{ $store->store_id }}"
                        @if($staff->store_id == $store->store_id) selected @endif>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="text" id="username" name="username" class="form-control" 
                    value="{{ $staff->username }}" required maxlength="16">
            </div>
        </div>
